...
@DateTime: 2026-02-08
@Docs: End-to-end tests for SQLModel example app.
SQLModel 示例应用的端到端测试。
"""
...
@pytest.mark.asyncio
class TestSQLModelE2E:
    """SQLModel end-to-end import/export tests.
    SQLModel 端到端导入导出测试。
    """
    ...
    async def test_commit_verifies_database(
        self, client: AsyncClient, csv_path: Path,
        ...
        async with factory() as session:
            ...
            assert {d.name for d in devices} == {"switch-01", "switch-02", "router-01", "ap-01", "firewall-01"}
    ...
    async def test_case_sensitivity_unique(
        self, client: AsyncClient, csv_path: Path, tmp_path: Path,
        db_session: tuple[async_sessionmaker[AsyncSession], str],
    ) -> None:
